  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        ...
        setIncome(dashboardRes.data.income);
        setOutcome(dashboardRes.data.outcome);
      ...
      <div className="flex w-full h-182.5">
        ...
        <div className="w-4/5 h-auto bg-[#e6e6e6] text-[#313638]">
          ...
          <div className="mx-10">
            ...
            <div className="w-full h-87.5 bg-[#fefefe] rounded-md p-5 mt-6 flex gap-5">
              {/* Graph */}
              ...
              <div className="flex flex-col w-2/5 h-full gap-12">
                {/* Income */}
                <div className="w-full border border-[#dbdbdb] bg-[#fefefe] rounded-md p-4 shadow-sm">
                  ...
                  <h2 className="text-xl font-bold text-green-600">
                    Rp {Number(income || 0).toLocaleString("id-ID")}
                  </h2>
                </div>

                {/* Expense */}
                <div className="w-full border border-[#dbdbdb] bg-[#fefefe] rounded-md p-4 shadow-sm">
                  <p className="text-lg opacity-80 mb-4">Expense</p>
                  <h2 className="text-xl font-bold text-red-600">
                    Rp {Number(outcome || 0).toLocaleString("id-ID")}
                  </h2>
                </div>
              </div>
            </div>
