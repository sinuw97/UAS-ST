...
const Sidebar = () => {
  ...
  const menuItems = [
    ...
    { name: "History Pembelian", path: "/history", icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div className="flex flex-col h-screen w-64 bg-white border-r border-teal-100 shadow-xl">
          ...
          {menuItems.map((item) => {
            ...
            return (
              <li key={item.name}>
                ...
                >
                  <span className={`transition-colors duration-200 ${isActive ? "text-teal-600" : "text-gray-400 group-hover:text-teal-500"}`}>
                    {item.icon}
                  </span>
      ...
      <div className="p-4 m-4 bg-teal-50 rounded-2xl border border-teal-100">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <p className="text-xs font-medium text-gray-500">System A Balance</p>
