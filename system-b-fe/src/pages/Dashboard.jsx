    ...
    <div className="flex h-screen bg-gray-50 overflow-hidden font-sans">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <header className="bg-white p-6 shadow-sm z-10 flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-teal-800">Selamat Datang, {userData.name} </h1>
            <p className="text-gray-500 text-sm mt-1">Siap berbelanja kebutuhan hari ini?</p>
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <button onClick={() => navigate('/cart')} className="relative p-2 text-gray-400 hover:text-teal-600 transition-colors">
              {cart.length > 0 && (
        ...
        <div className="flex-1 overflow-y-auto p-8">
            ...
            <div className="bg-gradient-to-r from-teal-500 to-emerald-600 rounded-3xl p-8 mb-10 text-white shadow-lg relative overflow-hidden">
                <div className="relative z-10 max-w-lg">
                    <h2 className="text-3xl font-bold mb-4">Belanja Sayur Segar Langsung dari Petani</h2>
                    <p className="mb-6 opacity-90">Dapatkan diskon 50% untuk pembelian pertama Anda menggunakan System A Balance.</p>
            ...
            <div className="mb-8">
                <div className="flex justify-between items-end mb-4">
                    ...
                    <a href="#" className="text-teal-600 text-sm font-semibold hover:underline">Lihat Semua</a>
                </div>
             ...
             <div>
                <h3 className="text-xl font-bold text-gray-800 mb-6 flex items-center gap-2">
                    <span>Produk {selectedCategory === "Semua" ? "Terlaris" : selectedCategory}</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {filteredProducts.map((product) => (
                        <div key={product.id} className="bg-white rounded-2xl p-4 shadow-sm hover:shadow-md transition-shadow border border-gray-100 group">
                            <div className="relative h-48 bg-gray-100 rounded-xl mb-4 overflow-hidden flex items-center justify-center">
                                ...
                                <button className="absolute top-3 right-3 p-2 bg-white rounded-full shadow-sm text-gray-400 hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100" onClick={() => addToCart(product)}>
                                   <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <div>
                                <p className="text-xs text-teal-600 font-semibold mb-1 uppercase tracking-wide">{product.category}</p>
                                <h4 className="font-bold text-gray-800 mb-1">{product.name}</h4>
                                <div className="flex justify-between items-center mt-3">
                                    <p className="text-lg font-bold text-teal-700">Rp {product.price.toLocaleString("id-ID")}</p>
                                    <button 
